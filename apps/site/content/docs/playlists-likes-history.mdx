...
There is no separate playlist, likes, or history export. These records live in `parson-music.db`, so back up the Parson data directory to preserve them together. Restoring only the music files does not restore accounts, playlists, likes, or history.
