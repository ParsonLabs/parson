...
When you open lyrics, Parson checks its memory and persistent cache. If there is no valid cached result, the **Parson host** asks LRCLIB using the track title, artist, album, and duration. It tries an exact match first, then bounded search fallbacks. Connected browsers do not contact LRCLIB directly.
...
Parson does not read embedded lyric tags or `.lrc` files. LRCLIB is the only built-in provider. A custom provider endpoint and a lyrics-specific local-only switch are not currently available.
