...
Parson does **not** read embedded lyric tags or `.lrc` sidecar files. Lyrics come from LRCLIB and may be synced or plain; see [Lyrics](/docs/lyrics).

## Playlist files

Parson stores playlists in its database. Importing or exporting `.m3u`, `.m3u8`, `.pls`, or other playlist files is not currently supported. Back up playlists with the Parson data directory.
